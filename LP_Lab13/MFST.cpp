...
#pragma region CONSTRUCTORS
	...
	Mfst::Mfst(LT::LexTable& plex, GRBH::Greibach pgreibach) {
		greibach = pgreibach;
		lex = plex;
		lenta = new short[lenta_size = lex.size];
		for (int k = 0; k < lenta_size; k++)
			lenta[k] = GRBH::Rule::Chain::T(lex.table[k].lexema);
		lenta_position = 0;
...
#pragma endregion
	...
	bool Mfst::push_chain(GRBH::Rule::Chain chain) {
	...
	char* Mfst::getCSt(char* buf) {
		...
		for (int k = (signed)st.size() - 1; k >= 0; --k) {
			p = st.c[k];
			buf[st.size() - 1 - k] = GRBH::Rule::Chain::alphabet_to_char(p);
		}
		buf[st.size()] = '\0';
		return buf;
	}
	...
	char* Mfst::getCLenta(char* buf, short pos, short n) {
		...
		for (int i = pos; i < k; i++)
			buf[i - pos] = GRBH::Rule::Chain::alphabet_to_char(lenta[i]);
	...
	char* Mfst::getDiagnosis(short n, char* buf) {
		...
		if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0) {
			...
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: строка %d,	%s", err.id, lex.table[lpos].sn, err.msg);
			rc = buf;
		}
